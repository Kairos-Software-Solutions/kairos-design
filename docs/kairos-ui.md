...
| `DataTable` | Table on desktop, record cards below 768px. Sorting, paging, row selection, search and column visibility, all off `columns` plus a prop each. A column declares its role once and that drives the table, the card, and the order. Server-backed tables use `pageIndex`/`onPageChange`, `sort`/`onSortChange`, `manualPagination`, `manualSorting`, and `rowCount`; the caller owns fetching and ordering while the table keeps its accessible range and controls. `rowNavigation` opts into whole desktop-row navigation while preserving the identifier anchor, and `collapsibleCards` opts into collapsed detail cards that show every visible column when opened. |
...
| `ThemeToggle`, `ThemeSetting`, `useThemePreference`, `themeInitScript` | The theme control, in both placements. `ThemeToggle` ships the word beside the glyph because a lone sun or moon does not say whether it shows the theme you have or the one you would get; `labelled={false}` drops it to the glyph in a square box, for a control sitting in a row where the word would outrank the row. The title and the accessible name state both halves either way. See [adr/0010](adr/0010-let-the-theme-toggle-drop-its-word.md) |
...
`formatSourceMoney(decimal, currency)` from `kairos-design/format/money` displays a decimal string exactly, including up to six fractional digits, without floating-point conversion. It pads to the currency's minor units and rejects unsupported currencies. Use it for imported source amounts before the separate accounting precision gate.

For dense mobile records, `DataTable` accepts `collapsibleCards` and `getRowLabel(row)` for a plain collapsed record name. `rowNavigation` takes `getHref(row)` and `getAccessibleName(row)`; retain a real identifier anchor for keyboard and modified-click navigation. A supplied `onSortChange` owns the page reset alongside sorting; the table does not emit a second page callback for that transition.
